...
public class TokenAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest servletRequest, @NonNull HttpServletResponse httpServletResponse, FilterChain filterChain) throws ServletException, IOException, ServletException {
        ...
        if (StringUtils.hasText(token)) {
            ...
            //这里放入了两个示例token 仅供测试
            map.put("test_token1", new User("admin", new BCryptPasswordEncoder().encode("123456")));
            map.put("test_token2", new User("root", new BCryptPasswordEncoder().encode("123456")));

            // 这里模拟从redis获取token对应的用户信息
            User customUserDetail = map.get(token);
            if (customUserDetail != null) {
                UsernamePasswordAuthenticationToken authRequest = new UsernamePasswordAuthenticationToken(customUserDetail,
                        null);
